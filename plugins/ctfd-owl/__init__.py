    ...
    def new_container():
        try:
            ...
            if ControlUtil.frequency_limit():
                return jsonify({'success': False, 'msg': '开启实例频率过快，请1分钟后重试。'})
            # check whether exist container before
            existContainer = ControlUtil.get_container(user_id)
            if existContainer:
                return jsonify({'success': False, 'msg': '您当前已经启用了{}的实例，请您先销毁该实例。'.format(existContainer.challenge.name)})
            else:
                challenge_id = request.args.get('challenge_id')
                ControlUtil.check_challenge(challenge_id, user_id)
                configs = DBUtils.get_all_configs()
                ...
                try:
                    ...
                    return jsonify({'success': True, 'msg':'开启实例失败，请联系CTF组织者。'})
        except Exception as e:
            return jsonify({'success': False, 'msg': str(e)})
